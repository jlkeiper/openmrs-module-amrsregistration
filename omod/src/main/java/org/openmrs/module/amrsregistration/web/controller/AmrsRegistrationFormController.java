...
public class AmrsRegistrationFormController extends AbstractWizardFormController {
        ...
        protected void postProcessPage(HttpServletRequest request, Object command, Errors errors, int page) throws Exception {
                ...
                if (page == AmrsRegistrationConstants.EDIT_PAGE) {
                        ...
                        if (idCard == null) {
                                ...
                                } else // Preferred is not one of the name in the person object, so it's going to be on one of
        ...
        protected int getTargetPage(HttpServletRequest request, Object command, Errors errors, int page) {
                ...
                if (page == AmrsRegistrationConstants.START_PAGE) {
                        ...
                        if (idCard != null) {
                                ...
                                if (patients != null && patients.size() > 0) {
                                        ...
                                        if (patients.size() > 1) {
                                                ...
                                                if (targetIdentifierExists(patient)) // send user to review page if the target identifier is found
                                                {
                                                        targetPage = AmrsRegistrationConstants.REVIEW_PAGE;
                                                }
                                        }
                                } else // show error when no patient is found
                                {
                        ...
                        targetPage = AmrsRegistrationConstants.ASSIGN_ID_PAGE;
                        if (targetIdentifierExists(patient)) // jump to review if the selected patient already have the targeted amrs identifier
                        {
                                targetPage = AmrsRegistrationConstants.REVIEW_PAGE;
                        }
                        ...
                        if (patientId != null) {
                                ...
                                if (matchedPatient != null) {
                                        ...
                                        targetPage = AmrsRegistrationConstants.ASSIGN_ID_PAGE;
                                        if (targetIdentifierExists(patient)) // jump to review when the patient already have the targeted amrs identifier
                                        {
                                                targetPage = AmrsRegistrationConstants.REVIEW_PAGE;
                                        }
                                }
        ...
        private void validate(Object command, Errors errors, boolean finish) {
                ...
                for (PersonAddress address : patient.getAddresses()) {
                        ...
                        if (StringUtils.isBlank(address.getSubregion())
                                || StringUtils.isBlank(address.getRegion())) // TODO: probably need to do validation like above (name)
                        {
                                foundInvalid = true;
                        }
                }
        ...
        /**
         * handle the "cancel" command in this wizard
         * 
         * @param request
         * @param response
         * @param command
         * @param bindException
         * @return the next page to view
         * @throws Exception 
         */
        @Override
        ...
        /**
         * complete the registration process by saving the new patient
         * 
         * @param request
         * @param response
         * @param command
        ...
        protected ModelAndView processFinish(
                ...
                try {
                        AmrsRegistration amrsRegistration = (AmrsRegistration) command;
                        AmrsRegistrationService service = Context.getService(AmrsRegistrationService.class);

                        // render XML from the event; done first to avoid registration without a record of it
                        String xml = service.renderRegistrationXML(amrsRegistration);

                        // process the registration
                        service.register(amrsRegistration);

                        // save the rendered XML
                        service.saveRegistrationXMLToFile(xml);
                } catch (APIException e) {
                        throw new APIException("Could not process registration.", e);
                }
        ...
        /**
         * generate a new, blank patient
         * 
         * @return a blank patient
         */
        ...
        /**
         * duplicates a patient by copying properties
         * 
         * @param to destination patient
         * @param from source patient
         */
        ...
        /**
         * Creates a person's birthdate from potentially partial input data
         * 
         * @param person the person to affect
